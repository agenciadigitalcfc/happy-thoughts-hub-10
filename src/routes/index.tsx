...
import { Activity, HeartPulse, Quote, Ruler, Scissors, Sparkles, Target } from "lucide-react";
 ...
 return <main>
  <Spotlight className="relative overflow-hidden">
   ...
   <div className="relative mx-auto grid max-w-6xl items-center gap-12 px-6 pb-20 pt-36 md:pt-44 lg:grid-cols-2">
    ...
    <div className="relative mx-auto w-full max-w-sm"><div className="aspect-[4/5] overflow-hidden rounded-[2rem] border border-gold/30 bg-background/10 shadow-2xl"><img src="/dr-samuel-montalvao-hero.jpg" alt="Dr. Samuel Montalvão — Cirurgião-Dentista e Harmonização Orofacial" className="h-full w-full object-cover object-center" loading="eager" fetchPriority="high"/></div></div>
   </div>
  </Spotlight>
  ...
  <section className="px-6 py-24"><div className="mx-auto max-w-6xl"><div data-reveal><span className="eyebrow">{t("spec.eyebrow")}</span><h2 className="title-display mt-4 text-3xl md:text-5xl">{t("spec.title")}</h2></div><div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-4">{["procedure.harmonizacao","procedure.preenchimento","procedure.labial","procedure.rino","procedure.toxina","procedure.bio","procedure.pdo","procedure.bichectomia","procedure.regeneracao","procedure.capilar"].map((k,index)=>{const I=[Sparkles,Activity,Scissors,Target,Sparkles,HeartPulse,Activity,Scissors,HeartPulse,Target][index];return <a key={k} href={whatsappLink(`Olá! Gostaria de saber mais sobre: ${t(k+".t")}`)} target="_blank" rel="noopener noreferrer" className="vellum vellum-hover rounded-3xl p-7"><I className="size-6 text-gold"/><h3 className="title-display mt-6 text-xl">{t(k+".t")}</h3><p className="mt-2 text-sm text-text-soft">{t(k+".d")}</p></a>})}</div></div></section>
  ...
  <SuccessCases/><section className="px-6 py-24" id="avaliacao"><div className="mx-auto max-w-6xl grid gap-8 lg:grid-cols-2"><div><span className="eyebrow">Primeira consulta</span><h2 className="title-display mt-4 text-3xl md:text-5xl">Tudo começa com uma avaliação individualizada.</h2></div><div className="vellum rounded-3xl p-8"><p className="text-sm leading-relaxed text-text-soft">A primeira consulta é o momento de compreender as principais queixas, analisar o rosto, avaliar histórico, expectativas, possibilidades e prioridades para construir um planejamento personalizado.</p><div className="mt-7 grid gap-3 text-sm">{["Compreender queixas e expectativas","Analisar o rosto e o histórico","Identificar possibilidades e prioridades","Construir um planejamento personalizado"].map(x=><div key={x} className="flex gap-3"><span className="mt-2 size-1.5 shrink-0 rounded-full bg-gold"/>{x}</div>)}</div><a href={whatsappLink()} target="_blank" rel="noopener noreferrer" className="btn-gold-lux mt-8">Agendar avaliação</a></div></div></section><OurTeam/>

  <section className="px-6 py-24" id="depoimentos"><div className="mx-auto max-w-6xl"><div className="text-center"><span className="eyebrow">{t("test.eyebrow")}</span><h2 className="title-display mt-4 text-3xl md:text-5xl">{t("test.title")}</h2></div><div className="mt-12 grid gap-5 md:grid-cols-3">{["test.q1","test.q2","test.q3"].map(q=><div key={q} className="vellum rounded-3xl p-7"><Quote className="size-5 text-gold"/><p className="mt-5 font-display text-lg leading-snug">{t(q)}</p></div>)}</div></div></section>
